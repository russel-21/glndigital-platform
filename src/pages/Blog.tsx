...
const blogPosts: { id: string; fr: PostDetail; en: PostDetail }[] = [
  {
    ...
    en: {
      ...
      excerpt: "Discover the flaws of simple organic visibility and learn how to structure a real advertising system to attract qualified clients in Douala and Yaoundé.",
  ...
  },
  {
    ...
    en: {
      ...
      excerpt: "Wasting budget, wrong pixels, targeting too broad... Avoid these critical errors and learn to configure your ad campaigns with precise targeting.",
  ...
  },
  {
    ...
    en: {
      ...
      excerpt: "From ChatGPT to CapCut, discover how to optimize your production speed for visuals and short videos for your clients.",
      category: "Community Management",
      date: "May 12, 2026",
      ...
      <div className="container mx-auto px-4 md:px-8 max-w-5xl">
        ...
        >
          <span className="bg-primary/10 text-primary text-xs font-semibold px-4 py-1.5 rounded-full border border-primary/20">
            {language === "fr" ? "Blog & Astuces" : "Blog & Tips"}
          </span>
          <h1 className="font-heading text-4xl md:text-5xl font-bold mt-6 mb-4">
            {language === "fr" ? (
              ...
              <>The <span className="text-gradient-primary">Digital Marketing</span> Blog</>
            )}
          </h1>
          <p className="text-muted-foreground max-w-2xl mx-auto text-base">
            {language === "fr"
              ...
              : "Closing tips, Meta Ads tutorial campaigns, and practical community management guides to accelerate your business in Cameroon."}
          </p>
        </motion.div>
        ...
        <div className="space-y-8">
          {blogPosts.map((post, idx) => {
            const data = post[language] || post.fr;
            return (
              <motion.article
                key={post.id}
              ...
              >
                <div className="flex-1 space-y-4">
                  <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
                    <span className="text-primary font-bold bg-primary/10 px-2.5 py-1 rounded-full uppercase tracking-wider">{data.category}</span>
                  ...
                  <h2 className="font-heading text-xl md:text-2xl font-bold text-foreground leading-tight hover:text-primary transition-colors">
                    <Link to={`/blog/${post.id}`}>{data.title}</Link>
                  </h2>

                  <p className="text-muted-foreground text-sm leading-relaxed">{data.excerpt}</p>

                  <div className="flex items-center gap-2 pt-2 text-xs font-semibold text-foreground">
                    <User className="w-4 h-4 text-primary" />
                    <span>
                      {language === "fr" ? `Publié par ${data.author}` : `Published by ${data.author}`}
                    </span>
                  </div>
                </div>
